...
class HFDatasetsData(CustomObject):
    ...
    def _get_ds_meta(self, dataset: DatasetDict):
        ...
        return {
            ...
            'features': [{feature: str(dataset_info['features'][feature])} for feature in dataset_info['features'].keys()],
            'post_processed': str(dataset_info['post_processed']),
            'supervised_keys': str(dataset_info['supervised_keys']),
            'task_templates': [str(template) for template in dataset_info['task_templates']],
