...
from typing import TypeVar, Generic, Any, Optional, Dict, Tuple, List, Iterator, Callable
...
class Sequence(Generic[ItemType], ABCSequence):
    ...
    def __init__(self, container: 'Container', *, name: str, context: Dict):
        self._container: 'Container' = container
        self._context_tree = self._container._meta_tree.subtree(KeyNames.CONTEXTS)
        self._data_loader: Callable[[], 'TreeView'] = self._container._data_loader
        self.__data = None

        self._name = name
        self._context = Context(context)
        self._ctx_id = self._context.idx

        self._meta_tree = self._container._tree.subtree((KeyNames.SEQUENCES, self._ctx_id, self._name))

        self._values = None
        self._axis = {}
        self._info = _SequenceInfo(self._meta_tree)
    ...
    def track(self, value: Any, *, step: Optional[int] = None, **axis):
        ...
        if self._info.empty:
            self._context_tree[self._ctx_id] = self._context.to_dict()
            self._meta_tree[KeyNames.INFO_PREFIX, 'creation_time'] = utc_timestamp()
            self._meta_tree[KeyNames.INFO_PREFIX, 'version'] = self.version
            self._meta_tree[KeyNames.INFO_PREFIX, 'first_step'] = self._info.first_step = step
            self._meta_tree[KeyNames.INFO_PREFIX, 'last_step'] = self._info.last_step = step
            self._meta_tree[KeyNames.INFO_PREFIX, 'axis'] = self._info.axis_names = tuple(axis.keys())
            self._meta_tree[KeyNames.INFO_PREFIX, KeyNames.VALUE_TYPE] = self._info.dtype = value_type
            self._meta_tree[KeyNames.INFO_PREFIX, KeyNames.SEQUENCE_TYPE] = self.get_full_typename()

            self._meta_tree['first_value'] = value
            self._meta_tree['last_value'] = value
            self._info.empty = False

        if step > self._info.last_step:
            self._meta_tree[KeyNames.INFO_PREFIX, 'last_step'] = step
            self._meta_tree['last_value'] = value
            self._info.last_step = step
        if step < self._info.first_step:
            self._meta_tree[KeyNames.INFO_PREFIX, 'first_step'] = step
            self._meta_tree['first_value'] = value
        ...
        if not type_utils.is_subtype(value_type, self._info.dtype):
            ...
            self._meta_tree[KeyNames.INFO_PREFIX, KeyNames.VALUE_TYPE] = self._info.dtype = dtype
    ...
    def _data(self) -> 'TreeView':
        if self.__data is None:
            self.__data = self._data_loader().subtree((self._ctx_id, self._name))
        return self.__data
    ...
    class QueryProxy:
        def __init__(self, sequence: 'Sequence', cache: Dict):
            self._cache = cache[sequence._container.hash]
            self._sequence_obj = sequence

        @property
        def name(self):
            return self._sequence_obj.name

        @property
        def context(self):
            from aim.storage.proxy import AimObjectProxy
            from aim.sdk.core.query_utils import ContextDictView

            return AimObjectProxy(lambda: self._context, view=ContextDictView(self._sequence_obj.context))
        ...
        def __getattr__(self, item):
            ...
            def safe_collect():
                try:
                    return self._sequence_obj._meta_tree.collect(item)
                except Exception:
                    return SafeNone()

            return AimObjectProxy(safe_collect, view=self._sequence_obj._meta_tree.subtree(item))

    def _proxy(self, query_cache) -> QueryProxy:
        return Sequence.QueryProxy(self, query_cache)
    ...
    def _check(self, query, query_cache, *, aliases=()) -> bool:
        ...
        container_proxy = self._container._proxy(query_cache)
        container_alias_names = self._container.default_aliases

        query_params = {p: proxy for p in alias_names}
        query_params.update({cp: container_proxy for cp in container_alias_names})
        return query.check(**query_params)

    def __repr__(self) -> str:
        return f'<{self.get_typename()} #{hash(self)} name={self.name} context={self._ctx_id}>'
