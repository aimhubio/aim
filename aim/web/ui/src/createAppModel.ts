...
import onModelNotificationAdd from './utils/app/onNotificationAdd';
...
import onModelBookmarkCreate from './utils/app/onBookmarkCreate';
import onModelNotificationDelete from './utils/app/onNotificationDelete';
import { INotification } from './types/components/NotificationContainer/NotificationContainer';
import onModelBookmarkUpdate from './utils/app/onBookmarkUpdate';
import onModelResetConfigData from './utils/app/onResetConfigData';
...
function createAppModel(appInitialConfig: IAppInitialConfig) {
  ...
  function getMetricsAppModelMethods(appInitialConfig: IAppInitialConfig) {
    const { appName, grouping, components, selectForm } = appInitialConfig;
              ...
              rows[groupKey!].items.push(
                isRowData
                  ...
                  : metricsTableRowRenderer(rowValues, {
                      toggleVisibility: (e) => {
                        e.stopPropagation();
                        onRowVisibilityChange(rowValues.key, model, appName);
                      },
                    }),
              );
            } else {
      ...
      const processedData = groupData(
        _.orderBy(
          ...
          configData?.table?.sortFields?.map(
            (f: SortField) => (metric: IMetric) => _.get(metric, f[0], ''),
          ) ?? [],
    ...
    function updateURL(configData = model.getState()?.config || {}) {
      ...
      updateUrlParam(encodedParams, appName);
    }
    ...
    function onBookmarkCreate({ name, description }: any) {
      return onModelBookmarkCreate({ name, description }, model, appName);
    }

    function onBookmarkUpdate(id: string) {
      onModelBookmarkUpdate(id, model, appName);
    }

    function onNotificationDelete(id: number) {
      onModelNotificationDelete(id, model);
    }

    function onNotificationAdd<M, N>(notification: N & INotification) {
      onModelNotificationAdd(notification, model);
    }

    function onResetConfigData() {
      onModelResetConfigData(model);
    }
    ...
    if (selectForm) {
      Object.assign(methods, {
        onMetricsSelectChange<M, D>(data: D & Partial<ISelectMetricsOption[]>) {
  ...
  function getRunsAppModelMethods(appInitialConfig: IAppInitialConfig) {
    const { appName } = appInitialConfig;

    if (appName === AppNameEnum.PARAMS) {
      return getParamsModelMethods(appInitialConfig);
    } else if (appName === AppNameEnum.RUNS) {
      return getRunsModelMethods(appInitialConfig);
    }
  }

  function getRunsModelMethods(appInitialConfig: IAppInitialConfig) {
    const { appName, grouping, components, selectForm } = appInitialConfig;
    ...
    function initialize(appId: string = '') {
      ...
      if (!appId) {
        ...
        if (searchFromUrl) {
          setItem(`${appName}Url`, getUrlWithParam({ search: searchFromUrl }));
        } else {
          if (urlFromStorage) {
    ...
    if (grouping) {
      // Object.assign(methods, {} as typeof methods);
    }
    if (selectForm) {
      Object.assign(methods, {
        onSelectRunQueryChange(query: string) {
        ...
        updateSelectStateUrl(): void {
          ...
          if (selectData) {
            updateUrlParam({ search: selectData }, appName);
          }
        },
      } as typeof methods);
    }
  ...
  function getParamsModelMethods(appInitialConfig: IAppInitialConfig) {
    const { appName, grouping, components, selectForm } = appInitialConfig;
      ...
      return {
        call: async () => {
          if (_.isEmpty(configData?.select?.params)) {
            ...
            if (
              Array.isArray(components?.charts) &&
              components?.charts?.indexOf(ChartTypeEnum.HighPlot) !== -1
            ) {
              state.highPlotData = [];
            }
            if (components.table) {
    ...
    function getDataAsLines(
      ...
      if (
        !processedData ||
        (configData?.select?.params && _.isEmpty(configData.select.params))
      ) {
        return [];
      }
  ...
  function getAppModelMethods(appInitialConfig: IAppInitialConfig) {
    switch (appInitialConfig.dataType) {
      case AppDataTypeEnum.METRICS:
  ...
  return {
    ...model,
    ...getAppModelMethods(appInitialConfig),
  };
}
