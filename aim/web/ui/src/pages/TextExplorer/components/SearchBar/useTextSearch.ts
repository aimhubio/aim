...
function useTextSearch({
  ...
  function filterByCase(
    ...
    return rawData?.filter((item: { [key: string]: string }) => {
      switch (matchType) {
        case MatchTypes.Word:
          if (item[searchKey].search(appliedRegExp!) > -1) {
            return item;
          }
          break;
        case MatchTypes.Case:
          if (item[searchKey].indexOf(search) > -1) {
            return item;
          }
          break;
        case MatchTypes.RegExp:
          try {
            if (appliedRegExp!.test(item[searchKey])) {
              return item;
            }
          } catch (e) {
            setFilterOptions((fO) => ({
        ...
        default:
          if (
            item[searchKey].toLowerCase().indexOf(search.toLowerCase()) > -1
          ) {
            return item;
          }
      }
