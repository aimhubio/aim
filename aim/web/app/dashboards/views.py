...
async def dashboards_post_api(request: Request):
...
async def dashboards_put_api(dashboard_id: str, request: Request):
    with get_session() as session:
        dashboard = session.query(Dashboard) \
            .filter(Dashboard.uuid == dashboard_id, Dashboard.is_archived == False) \
            .first()  # noqa
...
async def dashboards_delete_api(dashboard_id: str):
    with get_session() as session:
        dashboard = session.query(Dashboard) \
            .filter(Dashboard.uuid == dashboard_id, Dashboard.is_archived == False) \
            .first()  # noqa
