...
def collect_runs_data(query):
    ...
    runs = repo.query_runs(query, report_mode=0)
    runs_dict = {}
...
def collect_metrics_data(query):
    repo = Repo.default_repo()
    runs_dict = {}
    runs = repo.query_metrics(query=query, report_mode=0)
    for run_trace_collection in runs.iter_runs():
        run = None
        traces_list = []
...
def query_runs(query):
    repo = Repo.default_repo()
    runs = list(repo.query_runs(query=query, report_mode=0).iter_runs())


@timing()
def query_metrics(query):
    repo = Repo.default_repo()
    metrics = list(repo.query_metrics(query=query, report_mode=0).iter())
